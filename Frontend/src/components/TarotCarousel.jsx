...
export default function TarotCarouselOfSix() {
...
<>
    ...
    <div className="absolute inset-0 bg-[#2B0E67]/22 backdrop-blur-lg z-0"></div>
       ...
       <div className="relative z-10 flex flex-col p-6 text-2xl text-black text-white font-playwrite">
        ...
        <h2 className='relative z-10 flex justify-center text-2xl text-black text-white font-playwrite'>Choose 3 of ypur tarot Selections</h2>
...
{ !loading && (
  <div className="relative z-10 w-full mt-24 text-center text-white -ml-[2%]">
    <h2 className="flex justify-center text-lg font-semibold font-modern">
      Scroll to view your results after your 3 selections
    </h2>
  </div>
)}
...
{!loading && revealed.length === 3 && (
  <>
  <div className="flex ml-12 justify-center items-center mt-20 w-[90%]">
    {["အတိတ်", "ပစ္စုပ္ပန်", "အနာဂတ်"].map((phase, i) => (
      <div key={phase} className="flex flex-col items-center justify-start w-[480px] m-4 space-x-4">
        <div className="flex flex-col items-center justify-center mr-6">
          <h3 className="mb-2 text-xl font-semibold capitalize">{phase}</h3>
          <img
            src={cards[revealed[i]].img}
            alt={cards[revealed[i]].name}
            className="object-cover w-32 h-48 mx-auto mb-2 rounded-md"
          />
        </div>
        <div>
          <p className="flex justify-center pb-8 -ml-2 text-2xl">{cards[revealed[i]].name}</p>
          <p className="text-lg">{cards[revealed[i]].role[phase]}</p>
        </div>
      </div>
